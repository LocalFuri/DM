...
public class ViewportLayoutEditor : EditorWindow
{
  ...
  // The original D1 side box is 16x19. Our extracted native side PNG is 14x19,
  ...
  private void BlitHookStyleD1SidesIntoPreview(Color32[] pixels)
  {
    ...
    for (int i = 0; i < previewWallOrnaments.Length; i++)
    {
      ...
      // Hook and Wood Ring both use the original coordinate-set-0 side
      // projection. Prefer a dedicated Wood Ring side texture when present;
      // otherwise use the already extracted 14x19 side silhouette.
      ...
      if (sideTexture == null
          ...
          || sideTexture.width != 14
          || sideTexture.height != 19)
      {
        continue;
      }
