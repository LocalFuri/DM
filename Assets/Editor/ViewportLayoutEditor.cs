...
public class ViewportLayoutEditor : EditorWindow
{
  ...
  // Original DOS Grate front at distance 2. In the supplied (16,8) West
  // screenshot Grate_Front2_21x13.png is an exact pixel match at screen
  // top-left (102,113). The wall is two steps ahead, (14,8) East.
  ...
  // Original DOS Grate D3 side. In the supplied (16,9) West screenshot
  // Grate_Side3_4x4.png occupies the left D3 side slot at screen top-left
  // (81,96). That face is wall tile (13,10) North. The art is the left-wall
  // perspective, so only the right blit is mirrored.
  // Framebuffer Y = 200 - 96 - 4 = 100.
  private const int GrateD3SideLeftX = 81;
  private const int GrateD3SideRightX =
      DungeonViewportWidth - GrateD3SideLeftX - 4;
  private const int GrateD3SideY = 100;
  ...
  // Hook now uses native Hook_Side_16x19.png 1:1 in that box.
  // Wood Ring still uses the 14x19 cutout centered one pixel inside it.
  // Texture2D framebuffer Y = 200 - 74 - 19 = 107.
  ...
  private const int HookD1SideRightX =
      DungeonViewportWidth - HookD1SideLeftX - 16;
  private const int HookD1SideY = 107;
  private const int WoodRingD1SideLeftX = 49;
  private const int WoodRingD1SideRightX =
      DungeonViewportWidth - WoodRingD1SideLeftX - 14;
  ...
  private const int HookD2SideRightX =
      DungeonViewportWidth - HookD2SideLeftX - HookD2SideWidth;
  private const int HookD2SideY = WoodRingD2SideY;
  private const int HookD3SideWidth = 3;
  private const int HookD3SideHeight = 5;
  private const int HookD3SideLeftX = WoodRingD3SideLeftX;
  private const int HookD3SideRightX =
      DungeonViewportWidth - HookD3SideLeftX - HookD3SideWidth;
  private const int HookD3SideY = WoodRingD3SideY;
  ...
  [System.NonSerialized]
  private Texture2D cachedGrateSide3Texture;
  [System.NonSerialized]
  ...
  [System.NonSerialized]
  private Texture2D cachedWoodRingSide2Texture;
  [System.NonSerialized]
  ...
  //
  // This is the generic Viewport-17 geometry model used by production wall
  // selection/rendering.
  //
...
// -------------------------------------------------------------------------
 ...
 // -------------------------------------------------------------------------
  ...
  // coordinate spaces explicitly on every command.
  //
      ...
      out int displayY)
  {
    ...
    // references as well, so ViewEdit cards and the renderer use the same
  ...
  private void ComposeEditModePreview()
  {
    ...
    BlitWoodRingD3SidesIntoPreview(pixels);
    BlitHookD3SidesIntoPreview(pixels);
    BlitGrateD3SidesIntoPreview(pixels);
  ...
  {
    // Load HallOfChampions.json (including championMirrors), but do not make
    // the F3 decoration depend on the minimap tile classifier.  The mirror
    // placement entry is the authoritative wall-decoration record.
    EnsurePreviewMiniMapLoaded();
    ...
    int expectedMirrorX = previewX + forwardX * 3;
    int expectedMirrorY = previewY + forwardY * 3;
    string expectedWallSide = FacingName(OppositePreviewFacing(previewFacing));
    ...
    for (int i = 0; i < previewChampionMirrors.Length; i++)
    {
      ...
      // Generic F3-front test: the stored Champion-mirror wall is exactly
      // three map cells in the viewing direction and faces back at the party.
      // Example: 9,7 South -> SYRA mirror at 9,10 North.
      if (mirror.x != expectedMirrorX
          || mirror.y != expectedMirrorY
          || !string.Equals(
              mirror.wall,
              expectedWallSide,
              System.StringComparison.OrdinalIgnoreCase))
  ...
  private void BlitChampionMirrorD2LeftIntoPreview(Color32[] pixels)
  {
    ...
    Texture2D sideMirror = GetChampionMirrorSideD2Texture();
    if (sideMirror == null || !sideMirror.isReadable)
      return;
  ...
  private void BlitChampionMirrorD2RightIntoPreview(Color32[] pixels)
  {
    ...
    Texture2D rightMirror = GetChampionMirrorSideD2Texture();
  ...
  private Texture2D GetChampionMirrorSideD2Texture()
  {
  ...
  private Texture2D GetWoodRingSideTexture()
  {
    if (cachedWoodRingSideTexture != null)
      return cachedWoodRingSideTexture;
    ...
    for (int i = 0; i < guids.Length; i++)
    {
      ...
      if (candidate.width == 14 && candidate.height == 19)
      {
        cachedWoodRingSideTexture = candidate;
        return candidate;
      }
    ...
    cachedWoodRingSideTexture = fallback;
    return fallback;
  }

  private Texture2D GetWoodRingSide2Texture()
  {
  ...
  /// <summary>
  /// Loads resolved Level 0 wall/floor ornaments from the generated all-level
  /// placement database. This is the authoritative path when the database is
  /// present; the old Hall-specific derivation remains only as fallback.
  /// </summary>
  private bool LoadPreviewFeaturesFromDatabase()
  {
    ...
    try
    {
      ...
      for (int i = 0; i < database.ornamentPlacements.Length; i++)
      {
        ...
        string wallName = NormaliseFeatureFaceName(
        ...
        wallOrnaments.Add(
            new WallOrnamentPlacement
            {
              type = NormaliseFeatureTypeName(placement.resolved.type),
              ornamentOrdinal = placement.ornamentOrdinal,
              x = placement.normalisedX,
              y = placement.normalisedY,
              wall = wallName,
              wallTilePlacement = true
            });
      }

      previewWallOrnaments = wallOrnaments.ToArray();
      return true;
    }
    catch (System.Exception ex)
    {
      Debug.LogWarning(
          "ViewEdit feature database load failed; using Hall fallback: "
          + ex.Message);
      return false;
    }
  }

  private static string NormaliseFeatureTypeName(string type)
  {
    if (string.Equals(
            type,
            "WallGrate",
  ...
  private static string NormaliseFeatureFaceName(string face)
  {
    if (string.IsNullOrEmpty(face))
      return null;

    switch (char.ToUpperInvariant(face[0]))
    {
      case 'N': return "North";
      case 'E': return "East";
      case 'S': return "South";
  ...
  private void BlitViAltarD1FrontIntoPreview(Color32[] pixels)
  {
    ...
    Texture2D altar = GetViAltarFrontTexture();
  ...
  private Texture2D GetViAltarFrontTexture()
  {
    if (cachedViAltarFrontTexture != null)
    ...
    for (int i = 0; i < guids.Length; i++)
    {
      ...
      if (lower.Contains("96x56") || lower.Contains("96,56")) score += 25;
      if (score <= bestScore)
        continue;
  ...
  private void BlitWoodRingD3SidesIntoPreview(Color32[] pixels)
  {
    ...
    Texture2D side3 = GetWoodRingSide3Texture();
    if (side3 == null
        || !side3.isReadable
        || side3.width != 3
        || side3.height != 5)
    ...
    // D3 side face is visible only through open D1, D2 and D3 center cells.
    int d1CenterX = previewX + forwardX;
    int d1CenterY = previewY + forwardY;
    int d2CenterX = previewX + forwardX * 2;
    int d2CenterY = previewY + forwardY * 2;
    ...
    if (!previewMiniMap.IsInside(d1CenterX, d1CenterY)
        ...
        || previewMiniMap.GetTile(d3CenterX, d3CenterY).Type
            == DungeonTileType.Wall)
    {
      return;
    }

    int leftWallTileX = d3CenterX - rightX;
    int leftWallTileY = d3CenterY - rightY;
    int rightWallTileX = d3CenterX + rightX;
    int rightWallTileY = d3CenterY + rightY;
    ...
    for (int i = 0; i < previewWallOrnaments.Length; i++)
    {
      ...
      else
      {
        matchesLeft =
            leftWallExists
            && ornament.x == d3CenterX
            && ornament.y == d3CenterY
            && string.Equals(
                ornament.wall,
                leftBoundaryDirection,
                System.StringComparison.OrdinalIgnoreCase);
  ...
  private void BlitWoodRingD2SidesIntoPreview(Color32[] pixels)
  {
    EnsurePreviewMiniMapLoaded();
    if (previewMiniMap == null
        || previewWallOrnaments == null
    ...
    Texture2D side2 = GetWoodRingSide2Texture();
    if (side2 == null
        || !side2.isReadable
        || side2.width != 5
        || side2.height != 10)
    ...
    // The D2 side face is visible only through open D1 and D2 center cells.
    int d1CenterX = previewX + forwardX;
    int d1CenterY = previewY + forwardY;
    int d2CenterX = previewX + forwardX * 2;
    int d2CenterY = previewY + forwardY * 2;
    ...
    // Screen-left solid wall faces inward with TurnRight(viewFacing), while
    // screen-right faces inward with TurnLeft(viewFacing).
    string leftPhysicalFace =
        FacingName(TurnPreviewFacingRight(previewFacing));
    string rightPhysicalFace =
        FacingName(TurnPreviewFacingLeft(previewFacing));

    // Keep compatibility with legacy/passable-cell Sensor representation.
    string leftBoundaryDirection =
        FacingName(TurnPreviewFacingLeft(previewFacing));
    string rightBoundaryDirection =
        FacingName(TurnPreviewFacingRight(previewFacing));
  ...
  private void BlitHookD3SidesIntoPreview(Color32[] pixels)
  {
    BlitHookSidesAtDistance(
        pixels,
        3,
        HookD3SideWidth,
        HookD3SideHeight,
  ...
  private void BlitHookD2SidesIntoPreview(Color32[] pixels)
  {
    BlitHookSidesAtDistance(
        pixels,
        2,
  ...
  private void BlitHookSidesAtDistance(
      Color32[] pixels,
      int distance,
      int destWidth,
      int destHeight,
      ...
      int y)
  {
    ...
    DungeonMap.GetRightOffset(
        previewFacing,
        out int rightOffsetX,
        out int rightOffsetY);
    ...
    if (!PreviewTileIsOpen(d1CenterX, d1CenterY)
        || !PreviewTileIsOpen(d2CenterX, d2CenterY)
        || (distance >= 3 && !PreviewTileIsOpen(d3CenterX, d3CenterY)))
    {
      return;
    }

    int laneCenterX = distance == 3 ? d3CenterX : d2CenterX;
    int laneCenterY = distance == 3 ? d3CenterY : d2CenterY;
    int leftWallTileX = laneCenterX - rightOffsetX;
    int leftWallTileY = laneCenterY - rightOffsetY;
    int rightWallTileX = laneCenterX + rightOffsetX;
    int rightWallTileY = laneCenterY + rightOffsetY;
    ...
    for (int i = 0; i < previewWallOrnaments.Length; i++)
    {
      ...
      else
      {
        matchesLeft =
            leftWallExists
            && ornament.x == laneCenterX
            && ornament.y == laneCenterY
            && string.Equals(
                ornament.wall,
                leftBoundaryDirection,
                System.StringComparison.OrdinalIgnoreCase);
  ...
  private void BlitHookStyleD1SidesIntoPreview(Color32[] pixels)
  {
    EnsurePreviewMiniMapLoaded();
    if (previewMiniMap == null
        || previewWallOrnaments == null
    ...
    // D1 side faces are only visible through an open D1 center square.
    int frontX = previewX + forwardX;
    int frontY = previewY + forwardY;
    if (!previewMiniMap.IsInside(frontX, frontY)
        || previewMiniMap.GetTile(frontX, frontY).Type == DungeonTileType.Wall)
    ...
    // Physical faces on the solid wall tiles.
    string leftPhysicalFace =
        FacingName(TurnPreviewFacingRight(previewFacing));
    string rightPhysicalFace =
        FacingName(TurnPreviewFacingLeft(previewFacing));

    // Equivalent directions when an explicit Sensor is stored on the
    // passable D1 center tile instead of on the wall tile itself.
    string leftBoundaryDirection =
        FacingName(TurnPreviewFacingLeft(previewFacing));
    string rightBoundaryDirection =
        FacingName(TurnPreviewFacingRight(previewFacing));
    ...
    for (int i = 0; i < previewWallOrnaments.Length; i++)
    {
      ...
      int sideY = HookD1SideY;
      int sideLeftX = HookD1SideLeftX;
      int sideRightX = HookD1SideRightX;
      int expectedWidth = 16;
      int expectedHeight = 19;
      if (IsHookOrnament(ornament))
      {
        sideTexture = GetHookSideTexture();
      }
      else if (IsWoodRingOrnament(ornament))
      {
        sideTexture = GetWoodRingSideTexture();
        sideY = WoodRingD1SideY;
        sideLeftX = WoodRingD1SideLeftX;
        sideRightX = WoodRingD1SideRightX;
        expectedWidth = 14;
      }
      else
      {
      ...
      if (sideTexture == null
          || !sideTexture.isReadable
          || sideTexture.width != expectedWidth
          || sideTexture.height != expectedHeight)
      {
        continue;
      }
  ...
  private void BlitGrateD3SidesIntoPreview(Color32[] pixels)
  {
    ...
    Texture2D side3 = GetGrateSide3Texture();
    if (side3 == null
        || !side3.isReadable
        || side3.width != 4
        || side3.height != 4)
    ...
    // D3 side faces are visible only through open D1, D2, and D3 centers.
    int d1CenterX = previewX + forwardX;
    int d1CenterY = previewY + forwardY;
    int d2CenterX = previewX + forwardX * 2;
    int d2CenterY = previewY + forwardY * 2;
  ...
  private Texture2D GetGrateSide3Texture()
  {
  ...
  private void BlitGrateD2FrontIntoPreview(Color32[] pixels)
  {
    ...
    Texture2D grateFront = GetGrateFront2Texture();
    if (grateFront == null
        || !grateFront.isReadable
        || grateFront.width != 21
        || grateFront.height != 13)
    {
      return;
    }
    ...
    // F2 front is the wall two steps ahead, seen through an open D1 cell.
    int openX = previewX + forwardX;
    int openY = previewY + forwardY;
  ...
  private Texture2D GetGrateFront2Texture()
  {
